...
function BlogForm({
  ...
  return (
    ...
    >
      ...
      <div><Label>Slug</Label><Input name="slug" defaultValue={initial?.slug ?? ""} placeholder="auto" /></div>
      <div><Label>Cover URL</Label><Input name="cover_url" defaultValue={initial?.cover_url ?? ""} /></div>
      <div><Label>Excerpt</Label><Textarea name="excerpt" rows={2} defaultValue={initial?.excerpt ?? ""} /></div>
      <div><Label>Body</Label><Textarea name="body" rows={8} defaultValue={initial?.body ?? ""} /></div>
      <div><Label>Tags (comma-separated)</Label><Input name="tags" defaultValue={(initial?.tags ?? []).join(", ")} /></div>
      <div>
        <Label>Status</Label>
...
export function BlogsTable() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { invalidate(); toast.success("Blog created"); setAdding(false); },
    onError: (e: any) => toast.error(e.message),
  });
  const updateMut = useMutation({
    mutationFn: (v: any) => update({ data: v }),
    onSuccess: () => { invalidate(); toast.success("Blog updated"); setEditing(null); },
    onError: (e: any) => toast.error(e.message),
  });
  const deleteMut = useMutation({
    mutationFn: (id: string) => del({ data: { id } }),
    onSuccess: () => { invalidate(); toast.success("Deleted"); setConfirmDel(null); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  if (error) return <div className="rounded-md border border-destructive bg-destructive/10 p-4 text-sm text-destructive">Failed to load blogs: {(error as Error).message}</div>;
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-md border">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Title</TableHead><TableHead>Status</TableHead><TableHead>Tags</TableHead><TableHead>Updated</TableHead><TableHead className="text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? Array.from({ length: 5 }).map((_, i) => <TableRow key={i}><TableCell colSpan={5}><Skeleton className="h-6 w-full" /></TableCell></TableRow>)
              : rows.length === 0 ? <TableRow><TableCell colSpan={5} className="py-8 text-center text-sm text-muted-foreground">No blog posts yet.</TableCell></TableRow>
              : rows.map((b) => (
                <TableRow key={b.id}>
                  <TableCell className="font-medium">{b.title}</TableCell>
                  <TableCell><Badge variant="outline">{b.status}</Badge></TableCell>
                  <TableCell className="text-xs text-muted-foreground">{(b.tags ?? []).join(", ") || "—"}</TableCell>
                  <TableCell className="text-xs text-muted-foreground">{new Date(b.updated_at ?? b.created_at).toLocaleDateString()}</TableCell>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {editing && <BlogForm initial={editing} onSubmit={(v) => updateMut.mutate(v)} pending={updateMut.isPending} />}
        </DialogContent>
      </Dialog>
