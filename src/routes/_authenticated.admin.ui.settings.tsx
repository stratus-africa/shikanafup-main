...
import { listSettings, upsertSetting, upsertSettings, deleteSetting } from "@/lib/admin/settings.functions";
...
function CampaignPopupCard({
  ...
  const savedPopupImage = valueOf("campaign_popup.image_url");
  const [popupImage, setPopupImage] = useState<string>(savedPopupImage);
  useEffect(() => setPopupImage(savedPopupImage), [savedPopupImage]);
  const selectPopupImage = (imageUrl: string) => {
    setPopupImage(imageUrl);
    onSave([{ key: "campaign_popup.image_url", value: imageUrl }]);
  };
  ...
  const saveAll = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updates = [
      ...
      ["campaign_popup.dismiss_hours", String(fd.get("dismiss_hours") ?? "0.5")],
    ] as const;

    onSave(updates.map(([key, value]) => ({ key, value: String(value) })));
  };
        ...
        <form onSubmit={saveAll} className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5 md:col-span-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                />
                <ImagePicker
                  onSelect={selectPopupImage}
                  label="Select from gallery"
                  allowUpload
                  uploadLabel="Replace with new upload"
                />
              </div>
            </div>
          ...
          {popupImage && (
            <div className="rounded-md border bg-slate-50 p-3">
              <img src={popupImage} alt="Campaign popup preview" className="mx-auto max-h-48 rounded-md object-cover" />
            </div>
          )}
...
const CONTACT_FIELDS: { key: keyof typeof SITE_DEFAULTS; label: string }[] = [
...
function WebsiteLogoCard({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Website logo</CardTitle>
      </CardHeader>
      <CardContent>
        <p className="mb-4 text-sm text-muted-foreground">Shown in the website header and footer.</p>
        <form
          className="space-y-1.5"
          onSubmit={(e) => {
            e.preventDefault();
            onSave({ key: "site.logo_url", value: logoUrl });
          }}
        >
          <Label htmlFor="site.logo_url">Website logo URL</Label>
          <div className="flex flex-wrap items-center gap-2">
            <Input
              id="site.logo_url"
              value={logoUrl}
...
function ContactsCard({
  ...
  return (
    <Card>
      ...
      <CardContent>
        <p className="mb-4 text-sm text-muted-foreground">
          Contact and social details shown across the public website.
        </p>
...
function Page() {
  ...
  const upsertMany = useServerFn(upsertSettings);
  ...
  const campaignMut = useMutation({
    mutationFn: (updates: { key: string; value: string }[]) => upsertMany({ data: updates }),
    onSuccess: () => {
      inv();
      qc.invalidateQueries({ queryKey: ["public", "site-settings"] });
      toast.success("Campaign popup settings saved");
    },
    onError: (e: any) => toast.error(e.message ?? "Could not save campaign popup settings"),
  });
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        <div className="grid gap-6 lg:grid-cols-2">
          <WebsiteLogoCard settings={data as any[]} onSave={(v) => upsertMut.mutate(v)} saving={upsertMut.isPending} />
          <ContactsCard settings={data as any[]} onSave={(v) => upsertMut.mutate(v)} saving={upsertMut.isPending} />
          <CampaignPopupCard
            settings={data as any[]}
            onSave={(updates) => campaignMut.mutate(updates)}
            saving={campaignMut.isPending}
          />
        </div>
